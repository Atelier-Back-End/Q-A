...
export let options = {
  stages: [
    { duration: '10s', target: 1 },   // 1 request per second
    ///{ duration: '30s', target: 10 },  // 10 requests per second
    //{ duration: '30s', target: 100 }, // 100 requests per second
    //{ duration: '30s', target: 1000 }, // 1K requests per second
  ],
};

export default function () {
  // Make a request to your service's API
  let response = http.put(`http://${process.env.HOST}:${process.env.PORT}/questions/3518973/helpful`);
